...
export default async function HomePage() {
  ...
  const [intentions, recent] = await Promise.all([
    listIntentions(),
  ...
  const quota =
    user.plan === "pro"
      ? "Plan Pro · generación ilimitada"
      : `${PLAN.free.monthlyCustomizations - left} de ${PLAN.free.monthlyCustomizations} personalizaciones este mes`;
      ...
      <div className="om-shell pb-24 pt-14">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-[clamp(30px,4vw,42px)]">¿Qué quieres lograr hoy?</h1>
          </div>
          <Link
            href="/planes"
        ...
        <div className="mb-[18px] flex flex-wrap items-baseline justify-between gap-4">
          <h2 className="m-0 text-[14px] font-medium uppercase tracking-[0.18em] text-muted-soft">
            Banco de intenciones
          </h2>
          <span className="text-[14px] text-muted-soft">Listas para escuchar, sin generar</span>
        </div>

        {intentions.length === 0 ? (
          <p className="om-card mb-14 px-6 py-8 text-[16px] leading-[1.6] text-muted">
            El banco todavía está vacío. Corre{" "}
            <code className="font-mono text-[14px] text-clay">npm run seed</code> para
            cargar las intenciones curadas.
          </p>
        ) : (
          <div className="mb-14 grid grid-cols-[repeat(auto-fill,minmax(232px,1fr))] gap-[14px]">
            {intentions.map((it) => (
              <Link
                key={it.id}
                href={`/personalizar?intencion=${encodeURIComponent(it.title)}&i=${it.slug}`}
              ...
              >
                ...
                <div className="mt-[26px] text-[20px] tracking-[-0.01em]">{it.title}</div>
                <div className="mt-1.5 text-[14px] text-muted-soft">
                  {it.durations.join(" · ")} min
                </div>
              </Link>
            ))}
          </div>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-[14px]">
              {recent.map((m) => (
                <MeditationRow
                  key={m.id}
                  meditation={m}
                  meta={`${formatDuration(m.duration_seconds)} · ${formatDate(m.created_at)}`}
                />
              ))}
            </div>
          </>
